...
import reducer, { actions, constants as types } from '../application'
...
test('stepNextPersist action', () => {
  window.fetch = jest.fn().mockImplementation(() => Promise.resolve(mockResponse(200, null, '{"application":{}}')))
  ...
  return store.dispatch(actions.stepNextPersist(to, step)).then(() => {
...
test('submitApplication action', () => {
  ...
  window.fetch = jest
    .fn()
    .mockImplementation(() => Promise.resolve(mockResponse(200, null, JSON.stringify(expectedPayload))))
  ...
  return store.dispatch(actions.submitApplication()).then(() => {
    // return of async actions
    expect(store.getActions()).toEqual(expectedActions)
  })
})
