...
import { ClosingDate } from './ClosingDateControl'
...
test('closing date control shows correct day, month and year values', () => {
  ...
  const component = mount(
    <Provider store={store}>
      <ClosingDate id="test" value={format(date, 'YYYY-MM-DD')} formModel="test" />
    </Provider>
  )
...
test('closing date shows empty inputs on bad input data', () => {
  const state = {}
  const store = configureStore(state)

  const component = mount(
    <Provider store={store}>
      <ClosingDate id="test" value="thisisnotarealdate" formModel="test" />
    </Provider>
  )
...
test('closing date control calls the onDateChange property on value changes', () => {
  const state = {}
  const store = configureStore(state)
  const date = new Date()
