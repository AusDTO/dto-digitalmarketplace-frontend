...
describe('getBriefLastQuestionDate', () => {
  test('When closing date is over a week away', () => {
    ...
    expect(lastQuestionDate).toEqual(format(nextWeekDay(addDays(referenceDate, 5)), 'YYYY-MM-DD'))
  })

  test('When closing date is between 2 and 5 days away', () => {
    const closingDate = nextWeekDay(addDays(referenceDate, 3))
    const lastQuestionDate = format(getBriefLastQuestionDate(closingDate, referenceDate), 'YYYY-MM-DD')
    expect(lastQuestionDate).toEqual(format(nextWeekDay(addDays(referenceDate, 2)), 'YYYY-MM-DD'))
  })
  ...
  test('When closing date is 2 days away', () => {
    ...
    if (
      ...
      expect(lastQuestionDate).toEqual(format(nextWeekDay(addDays(referenceDate, 1)), 'YYYY-MM-DD'))
    }
  })
})
